"""
Background noise manager using FFmpeg for robust audio processing - FIXED VERSION
"""
import os
import asyncio
import logging
...
class NoiseManager:
    """Manages background noise using FFmpeg for audio processing - FIXED VERSION"""
    ...
    def _get_noise_chunk(self, chunk_size):
        """Get noise chunk with looping and debugging"""
        if not self.noise_data:
            return None
        ...
        if self._chunk_count % 1000 == 0:
            logger.info(f"Noise chunk #{self._chunk_count}: position={self.current_position}/{data_len}")
        
        if self.current_position >= data_len:
            self.current_position = 0  # Loop back to start
            logger.info(f"Background noise LOOPED at chunk #{self._chunk_count}")
        ...
        if self.current_position + chunk_size <= data_len:
            ...
            chunk = first_part + second_part
            self.current_position = remaining
            logger.info(f"Background noise wrapped around - position reset to {self.current_position}")
        
        return chunk


    def _mix_mulaw_audio(self, agent_mulaw, noise_mulaw):
        """Mix agent audio with background noise at μ-law level - FIXED VERSION"""
        try:
            ...
            # FIXED: Better volume control and mixing
            mixed_samples = array.array('h')
            for i in range(len(agent_samples)):
                # Agent at full volume, noise at controlled volume
                agent_sample = agent_samples[i]
                # FIXED: Apply volume more effectively
                noise_sample = int(noise_samples[i] * min(self.volume, 2.0))  # Allow up to 2x volume
                
                # FIXED: Better mixing algorithm
                mixed_sample = agent_sample + noise_sample
                
                # FIXED: Soft clipping instead of hard clipping
                if mixed_sample > 32767:
                    mixed_sample = 32767 - int((mixed_sample - 32767) * 0.1)
                elif mixed_sample < -32768:
                    mixed_sample = -32768 + int((-32768 - mixed_sample) * 0.1)
    ...
    def get_background_chunk(self, chunk_size):
        ...
        with self.lock:
            ...
            if noise_chunk and self.volume > 0:
                # FIXED: Apply volume to background-only audio too
                try:
                    # Convert to PCM, apply volume, convert back
                    ...
                    for sample in samples:
                        adjusted = int(sample * min(self.volume, 2.0))
                    ...
                    logger.error(f"❌ Error adjusting background volume: {e}")
                    return noise_chunk
            
            return noise_chunk
    
    def update_settings(self, noise_type=None, volume=None, enabled=None):
        """Update noise settings dynamically - FIXED VERSION"""
        if enabled is not None:
            self.enabled = enabled
        
        if volume is not None:
            # FIXED: Allow higher volumes, don't clamp to 1.0
            self.volume = max(0.0, min(10.0, volume))  # Allow up to 10x volume
    ...
    def start(self):
        ...
        logger.info(f"🎵 Background noise started at volume {self.volume}")
