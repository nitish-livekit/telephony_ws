# """
# Enhanced WebSocket server with FIXED outbound flag setting
# """
# import asyncio
# import uuid
# import logging
...
#     """Enhanced WebSocket server manager with FIXED outbound detection"""
...
#         """Handle incoming WebSocket connections from Plivo - FIXED OUTBOUND DETECTION"""
#         handler = None
#         try:
...
#             # 🆕 CRITICAL FIX: Properly extract outbound_agent_exists flag
#             outbound_agent_exists = query.get("outbound_agent_exists", ["false"])[0].lower() == "true"

#             # Log call type detection
#             if outbound_agent_exists:
#                 logger.info(f"🔄 OUTBOUND CALL DETECTED - Agent '{agent_name}' already exists in room '{room_name}'")
#                 logger.info(f"🚫 Will NOT dispatch new agent - WebSocket will connect to existing session")
#             else:
#                 logger.info(f"📞 INBOUND CALL DETECTED - Will create new agent '{agent_name}' for room '{room_name}'")
            
#             logger.info(f"📞 Room: {room_name}")
#             logger.info(f"🤖 Raw agent parameter: {query.get('agent')}")
#             logger.info(f"🤖 Parsed agent name: {agent_name}")
#             logger.info(f"🔄 Outbound agent exists: {outbound_agent_exists}")
...
#             # 🆕 CRITICAL FIX: Create handler for Plivo WebSocket (ONLY ONCE)
#             logger.info(f"🆕 Creating handler with agent_name='{agent_name}', outbound={outbound_agent_exists}")
#             handler = TelephonyWebSocketHandler(room_name, websocket, agent_name, noise_settings)
            
#             # 🆕 CRITICAL FIX: Set the outbound flag IMMEDIATELY after creation
#             handler.outbound_agent_exists = outbound_agent_exists
#             logger.info(f"✅ Handler outbound flag set to: {handler.outbound_agent_exists}")
...
#                 call_type = "OUTBOUND" if getattr(handler, 'outbound_agent_exists', False) else "INBOUND"
#                 logger.info(f"🧹 Removed {call_type} handler from active list - {len(self.active_handlers)} handlers remaining")
...
#         logger.info("🔧 Ready for both INBOUND and OUTBOUND Plivo WebSocket connections")
#         logger.info(f"📋 INBOUND: ws://sbi.vaaniresearch.com:{WEBSOCKET_PORT}/?room=your_room_name")
#         logger.info(f"📋 OUTBOUND: ws://sbi.vaaniresearch.com:{WEBSOCKET_PORT}/?room=your_room_name&agent=YourAgent&outbound_agent_exists=true")
...
#         # Log handler types
#         inbound_count = sum(1 for h in active_handlers if not getattr(h, 'outbound_agent_exists', False))
#         outbound_count = sum(1 for h in active_handlers if getattr(h, 'outbound_agent_exists', False))
#         logger.info(f"   - {inbound_count} INBOUND handlers")
#         logger.info(f"   - {outbound_count} OUTBOUND handlers")
...
#         """Get statistics about active handlers"""
#         active_handlers = self.active_handlers.copy()
#         inbound_count = sum(1 for h in active_handlers if not getattr(h, 'outbound_agent_exists', False))
#         outbound_count = sum(1 for h in active_handlers if getattr(h, 'outbound_agent_exists', False))
        
#         return {
#             "total": len(active_handlers),
#             "inbound": inbound_count,
#             "outbound": outbound_count,
#             "shutting_down": self._shutdown_initiated
#         }
...
import websockets
from urllib.parse import urlparse, parse_qs
from config import WEBSOCKET_HOST, WEBSOCKET_PORT, get_agent_name
from telephony.websocket_handler import TelephonyWebSocketHandler
...
class WebSocketServerManager:
    ...
    async def handle_telephony_websocket(self, websocket, path):
        ...
        try:
            ...
            # NEW: Use get_agent_name function for consistent agent resolution
            raw_agent_name = query.get("agent", [None])[0] 
            agent_name = get_agent_name(raw_agent_name)
            ...
            logger.info(f"📞 Room: {room_name}")
            logger.info(f"🤖 Raw agent parameter: {query.get('agent')}")
            logger.info(f"🤖 Resolved agent name: '{agent_name}' (from {'URL param' if raw_agent_name else 'environment/default'})")
            logger.info(f"🔄 Outbound agent exists: {outbound_agent_exists}")
